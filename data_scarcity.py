...
def evaluation(device, model, lb, batch):
    ...
    return loss, score, y_true, y_pred

def append_result(csv_path: Path, seed: int, pretrain: bool, frozen: bool, labeled_percentage: float, f1_score: float):
    """Écrit les résultats dans le fichier CSV commun."""
    file_exists = os.path.exists(csv_path)
    with open(csv_path, mode='a', newline='') as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow(["seed", "pretrain", "frozen", "labeled_percentage", "F1_score"])
        writer.writerow([seed, pretrain, frozen, labeled_percentage, f1_score])
...
def load_dataset():
    ...
    dataset_name = downstream_config['dataset']['name']
...
def train(seed:int, labeled_percentage:float , pretrain:bool, finetune:bool, results_file: Path):
    ...
    for epoch in tqdm(range(1,epochs+1), desc="Overall Training"):
        ...
        for batch in train_loader:
            optimizer.zero_grad()
            loss, score, _, _ = evaluation(device=device, model=model, batch=batch, lb=lb)
    ...
    for batch in test_loader:
        loss, score, y_true, y_pred = evaluation(device=device, model=model, batch=batch, lb=lb)
    ...
    total_score = f1_score(all_targets, all_predictions, average='weighted', zero_division=0)
    print(f"Test loss = {test_loss:.4f}, Test F1 Score = {test_score:.4f}")

    # Sauvegarder les résultats dans le fichier CSV
    append_result(results_file, seed, pretrain, finetune, labeled_percentage, total_score)
...
if __name__ == "__main__" :
    ...
    # --- Configuration ---
    seeds = list(range(15))
    labeled_percentages = [0.025, 0.075]
    # labeled_percentages = [0.01, 0.05, 0.1, 0.15, 0.2]
    pretrain_opts = [True, False]
    finetune_opts = [True, False]

    combos = list(itertools.product(seeds, labeled_percentages, pretrain_opts, finetune_opts))
    total_experiments = len(combos)

    # --- Login wandb ---
    wandb.login(key='3e0e644169a93d59382823b35ef232fdb2b25d25')

    # --- Initialisation du suivi global ---
    wandb.init(project="downstream_diagnostics", 
               name="global_progress", 
               reinit=True)

    # --- Charger le dataset et les configurations ---
    dataset, collate_fn, pretrain_params, device, pretrain_model_name, batch_size, learning_rate, epochs, training_params, input_size = load_dataset()

    # --- Boucle sur les différentes configurations ---
    for i, (seed, labeled_percentage, pretrain, finetune) in enumerate(combos, start=1):
        train(
            seed=seed,
